...
import br.com.pos.unicamp.vrep.utils.WheelUtils;

public class SimpleWallFollower implements Behavior {

	private Engine engine;
	private Pioneer pioneer;

	public Engine createEngineWallFollower() throws IOException {
		engine = new Engine();
		engine.setName("WallFollower");
	...
	@Override
	public void initialize(Pioneer pioneer) {
	...
	public void keepGoing() {
		...
		try {
			...
			// Front
			engine.getInputVariable("right_front_sensor").setValue(rateSensor(6));
			engine.getInputVariable("right_front_sensor").setValue(rateSensor(7));

			// Sides
			//engine.getInputVariable("side_sensor").setValue(rateSensor(6));
			//engine.getInputVariable("side_sensor").setValue(rateSensor(7));
			engine.getInputVariable("side_sensor").setValue(rateSensor(8));
			

			// Rear
			engine.getInputVariable("side_sensor").setValue(rateSensor(9));
			

			engine.process();

			double linear = engine.getOutputValue("whell_linear_speed");
			double angular = engine.getOutputValue("whell_angular_speed");

			// pioneer.drive(linear, angular);
			float velocityLeft = (float) WheelUtils.vLToDrive(linear, angular) * 30;

			float velocityRight = (float) WheelUtils.vRToDrive(linear, angular) * 30;

			pioneer.getMotors().setVelocity(toFloat(velocityLeft), toFloat(velocityRight));
